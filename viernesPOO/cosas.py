...
class AyudanteProfesor(Alumno, Profesor):
    ...
    def __str__(self):
        return Alumno.__str__(self) + Profesor.__str__(self) + f"Horas de trabajo: {self.__numero_horas}"
    
    def dar_clase(self, materia):
        print(f"{self.nombre} esta dando {materia} del are {self.area} por {self.numero_horas} horas")
